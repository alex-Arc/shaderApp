...
void MakeViewRay(out vec3 eye, out vec3 ray)
{
    vec2 ooR = 1./u_resolution.xy;
    vec2 q = gl_FragCoord.xy * ooR;
    //	q.x = mod(q.x*3.,1.);	//tile it for split views!
    //	q.y = mod(q.y*2.,1.);	//tile it for split views!
    vec2 p =  2.*q -1.;
    p.x *= u_resolution.x * ooR.y;
    
    vec3 lookAt = vec3(0.,0.,0.);
    float t = 24.4 + u_time*0.01; //iGlobalTime; //sin(iGlobalTime*.5)*.5;
    eye = vec3(-2.5,0,2.5);
...
void main(  )
{
    ...
    MakeViewRay(mainViewP, mainViewD);
    ...
    //	float s = sin(iGlobalTime)*0.025 + 1.;
    //	s * t * u = 1;
    //  tu = 1/s
    //  t=u=sqrt(1/s);
    //	float tu = sqrt(1./s);
    ...
    for (int inst=0; inst<5; inst++) {
        ...
        vec3 ax=noise3(vec3(uv*2.5,u_time*0.25+A.x));
        viewD=rotation(length(ax)*(0.05),normalize(ax))*viewD;		//wibble the view ray to change bubble shape
        
        //	viewP += (noise3(vec3(iGlobalTime*A.y,A.x,A.z))*2.-1.)*vec3(1.,0.6,1.1);		//float around
        
        viewP += smf(vec3(u_time*A.y*.5,A.x,A.z), 0.51, 2.13, 2, 0.2)*vec3(1.,0.6,1.1);		//float around
        
        viewP.yz -= (mod(u_time*0.05 + float(inst)*.37,1.)*2.-1.)*2.5;		//float up
        ...
        if (tt.y > tt.x)
        {
            ...
            for (int i=0; i<2; i++)
            {
                ...
                hit_p.y += u_time*0.1;
                vec3 soap_col = Colorize(RotY(n*0.5,A.x+u_time*0.2)-hit_p*0.1);
                
                vec3 soap = soap_col * (e + .5*length(soap_col));
                
                
                vec3 r = reflect(-viewD,n);			//reflection
               // r = textureCube(iChannel0,r).xyz;
                r = pow(r,vec3(2.2));
