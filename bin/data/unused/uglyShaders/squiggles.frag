...
void main(  )
{
    ...
    vec3 col = vec3(0.0);
    float amp = 1.0;
    float size = 4.0 * (abs(fract(time*.01-.5)-.5)*50.0+1.0) + ((mouseX/u_resolution.x) * 200.0);
    float timeSlide = .05;
    ...
    for (int i = 0; i < 20; i++)
    {
        ...
        col += amp * c * ((wormCol * .1) + vec3(red, green, blue));
        amp *= .85;
        time -= timeSlide;
    }
    gl_FragColor = vec4(min(col, 1.0), 0.8);
}
