...
uniform float mouseX;//.1,3000]
uniform float mouseY;//.1,3000]
//uniforms
...
void main()
{
    ...
    float theta = (iMouse.x / u_resolution.x *2. - 1.)*PI;
    float phi = (iMouse.y / u_resolution.y - .5)*PI;
    ...
    if (abs(q.y-.42)*iResolution.y<.5)  gl_FragColor = vec4(.75);
    else if (transmittance<0.) 		    gl_FragColor = skyColor;
    else if (transmittance>1.) 		    gl_FragColor = vec4(1.,0.,0.,1.);
    else
    { // display: object
        Rz = 1.-exp(-8.*DENS*Rz);
        float alpha = Rz;
    ...
    //display: light
    float d = length(vec2(lightDir)-p)*u_resolution.x;
    float Z; if (lightDir.z>0.) Z=1.; else Z=0.;
    if (d<10.) gl_FragColor = vec4(1.,Z,.5,1.);
