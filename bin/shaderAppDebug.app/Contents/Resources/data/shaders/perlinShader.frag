...
uniform float u_time;//-0.01,0.01]
uniform float density;//.1,1.]
uniform float fRatio;//0.1,0.9]
uniform float FirstDivision;//1,10]
uniform bool bwSwitch;//]
uniform bool bgTransparent;//]
uniform float rotCenterX;//0,1]
uniform float rotCenterY;//0,1]
//uniforms
...
void main() {
    ...
    uv = (uv - vec2(rotCenterX*aspectRatio,rotCenterY))*n_tiles_level_1 + vec2(rotCenterX*aspectRatio,rotCenterY);
    
    mat2 m = fRatio*mat2( cos(u_time),sin(u_time),
    ...
    for (float i = 0.; i < NbScales; i++) {
        ...
        float crit = n_tiles_level_1 *q - u_resolution.x/LimitDetails;
