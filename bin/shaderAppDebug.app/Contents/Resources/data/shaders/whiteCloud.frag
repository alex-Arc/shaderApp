...
uniform float u_density;//0,1000]
uniform float u_shadows;//0,1]
uniform float u_color1;//0,1]
uniform float u_color2;//0,1]
//uniforms
...
{
    vec4 xp = vec4(p*0.4,u_time*0.1+noise(p));
...
void main()
{
    vec2 psuedoMouse=vec2(u_time,u_density);
    vec2 uv = gl_FragCoord.xy / u_resolution.xy - 0.5;
    uv.x = uv.x*u_resolution.x/u_resolution.y;
    vec2 m = psuedoMouse.xy / u_resolution.xy - 0.5;
    ...
    for (int i=0; i<quality; i++) //max 20 step raycast
    {
        ...
        float nv = density(p+d*hash(uv+vec2(u_time,dense_acc))*0.25)*density_coef*quality_coef;
    ...
    vec3 dense_color = (other_color*u_shadows); //color of the dark part of the cloud
    
    sky_color=sky_color*(1.0-uv.y*0.2);
    ...
    color =cc(color,u_color1,u_color2);
